...
// AERIAL_OCUDU P7 L2->L1: DL_TTI.request.
...
// cuPHY rejects startRb + nRb > carrier PRBs; OCUDU rounds nof_rbs up to a
// multiple of 4 (276 on a 273 PRB carrier). Clamp, keeping the multiple of 4.
static uint16_t csi_rs_clamp_nr_of_rbs(uint16_t start_rb, uint16_t nr_of_rbs,
                                       int cell_dl_grid)
{
    if (cell_dl_grid <= 0) return nr_of_rbs;
    uint16_t grid = (uint16_t)cell_dl_grid;
    if (start_rb >= grid) return 0;
    uint16_t max_rbs = (uint16_t)(grid - start_rb);
    if (nr_of_rbs > max_rbs) nr_of_rbs = max_rbs;
    return (uint16_t)(nr_of_rbs & ~3u);
}
                       ...
                       int cell_dl_grid)
{
    ...
    o->nr_of_rbs = csi_rs_clamp_nr_of_rbs((uint16_t)c->crb_start,
                                          (uint16_t)(c->crb_stop - c->crb_start),
                                          cell_dl_grid);
    o->csi_type = c->type;
    o->row = c->row;
...
int aerial_l2l1_dl_tti_request(struct AppContext* ctx,
                              const uint8_t* body, uint32_t body_len)
{
    ...
    for (unsigned i = 0; i < n_csi   && n < NFAPI_NR_MAX_DL_TTI_PDUS; ++i) map_csi_rs(&dl->dl_tti_request_body.dl_tti_pdu_list[n++], &csi[i], ctx->aerial_ocudu_ctx.cell_dl_grid);
    ...
    int rc = aerial_send_p7_msg(ctx, (nfapi_p7_message_header_t*)&dl->header);
    free(dl);
    return (rc == 0) ? 1 : -1;
}

// AERIAL_OCUDU P7 L2->L1: UL_DCI.request. Carries the
// connected-mode UL grant DCI on the PDCCH, sent as a separate P7 message.
int aerial_l2l1_ul_dci_request(struct AppContext* ctx,
                              const uint8_t* body, uint32_t body_len)
{
    ocudu_rd_t r;
    ocudu_rd_init(&r, body, body_len);
