...
// OAI_OCUDU P5 L1->L2 dispatcher + xSM put helper. Builds an OCUDU FAPI
...
static uint64_t l1l2_now_ns(void)
{
    struct timespec ts;
                     ...
                     const void* body, uint32_t body_len)
{
    ...
    ocudu_xsm_hdr_build((uint8_t*)dst_va, msg_type, body_len, l1l2_now_ns());
                                 ...
                                 uint8_t ocudu_msg_type, uint8_t error_code)
{
    switch (ocudu_msg_type) {
        case OCUDU_FAPI_PARAM_RESPONSE:
            return ocudu_l1l2_param_response(ctx, error_code);
        case OCUDU_FAPI_CONFIG_RESPONSE:
            return ocudu_l1l2_config_response(ctx, error_code);
        default:
