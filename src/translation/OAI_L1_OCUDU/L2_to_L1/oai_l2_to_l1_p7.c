...
// OAI_OCUDU P7 L2->L1 dispatcher: parse the OCUDU xSM data-plane message and
...
int ocudu_p7_translate_and_send(struct AppContext* ctx, struct oai_vnf* v,
                                ...
                                uint32_t size)
{
    if (ctx == NULL || v == NULL || src_va == NULL) {
        return -1;
    }
    ...
    switch (msg_type) {
        case OCUDU_FAPI_DL_TTI_REQUEST:
            return ocudu_l2l1_dl_tti_request(ctx, v, body, body_len);

        case OCUDU_FAPI_TX_DATA_REQUEST:
            return ocudu_l2l1_tx_data_request(ctx, v, body, body_len);

        case OCUDU_FAPI_UL_TTI_REQUEST:
            return ocudu_l2l1_ul_tti_request(ctx, v, body, body_len);

        case OCUDU_FAPI_UL_DCI_REQUEST:
            return ocudu_l2l1_ul_dci_request(ctx, v, body, body_len);
